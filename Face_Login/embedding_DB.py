...
def insert_user_embedding(user_data, face_id):
    ...
    is_admin = 1 if user_data["position"] == "관리자" else 0

    sql = "INSERT INTO UserData (UserName, Department, Position, FaceID, IsAdmin) VALUES (%s, %s, %s, %s, %s)"
    data = (user_data["name"], user_data["department"], user_data["position"], face_id_bytes, is_admin)
...
def capture_and_register(user_data=None, capture_time=10, delay=0.5):
    ...
    insert_user_embedding(user_data, mean_embedding)
    set_login_flag(user_data["name"], flag=1)
...
def set_logout(user_name: str):
    set_login_flag(user_name, flag=0)
